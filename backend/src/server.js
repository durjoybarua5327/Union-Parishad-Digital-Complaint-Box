...
router.post('/users/login', async (req, res, next) => {
  ...
  // Update user profile
  router.put('/users/:id', upload.single('profile_image'), async (req, res, next) => {
    ...
    try {
      ...
      if (ward) { fields.push('ward = ?'); values.push(ward); }
      if (role) { fields.push('role = ?'); values.push(role); }
      if (profileImagePath) { fields.push('profile_image = ?'); values.push(profileImagePath); }
      values.push(userId);

      await db.execute(`UPDATE users SET ${fields.join(', ')} WHERE id = ?`, values);

      // Return updated user
      const [rows] = await db.execute('SELECT id, name, email, role, ward, profile_image FROM users WHERE id = ?', [userId]);
      res.success(rows[0], 'User profile updated successfully');
    } catch (err) {
      next({ status: 500, message: 'Failed to update user', code: 'DB_ERROR' });
    }
  });
// --- COMPLAINTS ---
